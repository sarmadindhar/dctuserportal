...
import { setLocale , logout } from '../../store/actions';
...
const Header = (props: {
  ...
  const handleLogout = ()=>{
    ...
    dispatch(
      ...
      logout(auth.user.refresh.token, () => {
        navigate('/login');
      })
    );
  }
  ...
  return (
    <div className='w-full px-10 py-6 pb-0'>

 
    <header className="relative z-999 flex w-full bg-white rounded-xl">
      <div className="flex flex-grow items-center justify-between px-4 py-4 shadow-2 md:px-6 2xl:px-11 rounded-xl">
        ...
        <div className="flex items-center gap-3 2xsm:gap-7">
          <ul className="flex items-center gap-2 2xsm:gap-4">
            ...
            <a href="#"
               className="rounded-md bg-[#fff] px-2 py-1 ml-5 text-sm font-regular text-black border border-black hover:bg-black hover:text-white"
               onClick={() => {
                 setAppLocale()
               }}>{t(`locale.${localeName}`)}</a>
            {/* <!-- Dark Mode Toggler --> */}
            {/* <DarkModeSwitcher /> */}
            {/* <!-- Dark Mode Toggler --> */}
